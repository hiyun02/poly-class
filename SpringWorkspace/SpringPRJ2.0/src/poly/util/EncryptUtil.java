...
//   일반적인 암호화 알고리즘 SHA-256을 통해서만 암호화시킬 경우, 암호화 된 값만 보고 일반적인 비밀번호에 대한 값을 쉽게 예측이 가능함
//   따라서, 암호화할 때 암호화되는 값에 추가적인 문자열을 붙여서 함께 암호화를 진행함
   ...
   //AES128-CBC 암호화 알고리즘에 사용되는 키 (16자리 문자만 사용 가능)
   final static String key="PolyTechnic12345"; //16글자. 영문자 1글자 당 1바이트
